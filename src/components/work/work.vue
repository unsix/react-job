      ...
      ...mapMutations({
        ...
        setCompanyList: 'SET_COMPANYLIST'
			...
			judgeState() {
				...
				if(m === 0 && f === 0) {
					this.workList = ['处理审批', '发起审批','创建公司', '工作记录','通讯录','工人列表','工程列表','我发布的','我发出的合同','我收到的合同','我的作品']
				}
				if(m === 1 && f === 0) {
					this.workList = ['处理审批', '发起审批', '公司管理', '创建公司', '权限管理', '邀请同事', '工作记录', '通讯录','工人列表','工程列表','我发布的','我发出的合同','我收到的合同','我的作品']
				}
				if(m === 0 && f === 1) {
					this.workList = ['处理审批', '发起审批', '表单回执','创建公司', '邀请同事', '工作记录', '通讯录','工人列表','工程列表','我发布的','我发出的合同','我收到的合同','我的作品']
				}
				if(m === 1 && f === 1) {
					this.workList = ['处理审批', '发起审批', '公司管理', '创建公司', '权限管理', '表单回执', '工作记录', '邀请同事', '通讯录','工人列表','工程列表','我发布的','我发出的合同','我收到的合同','我的作品']
				}
			},
			changeWorkIndex(num){
				this.workIndex = num
